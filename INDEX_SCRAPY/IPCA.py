...
while limit <= 10:
    try:
        ...
        html = urlopen("http://www.ipeadata.gov.br/ExibeSerie.aspx?serid=36482&module=M")
...
def inserir(table_name="IPCA", data_insert="DATA"):
    c1 = sq3.connect("INDEX.db")
    c2 = c1.cursor()
...
def export_csv(table_name="TABELA", select="*"):
    ...
    data.to_csv(getcwd()+"\IPCA.csv", sep=";",index=False, doublequote=False, decimal=".")

criar(table_name="IPCA")
inserir(table_name="IPCA", data_insert=data)

answer = input("Você deseja exportar o arquivo para CSV?")

if answer.upper() == "SIM":
    export_csv(table_name="IPCA")
